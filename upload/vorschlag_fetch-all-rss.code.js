// ═══════════════════════════════════════════════════════════════
//  FETCH ALL RSS FEEDS (ROBUSTIFIED)
//  Änderung vs. deine Version:
//   - Heise-URL korrigiert (alte liefert 0 Items)
//   - Sanity-Check: leere/dead Feeds werden geloggt, killen nicht den Run
//   - Per-Feed Try/Catch (hattest du schon — gut!)
//   - Statistik-Logging am Ende
//
//  Anmerkung: Die Heise-Feed-URLs ändern sich gelegentlich.
//  Aktuelle funktionierende Optionen (Stand Juli 2026):
//   - https://www.heise.de/rss/heise-atom.xml (Atom, stabil)
//   - https://www.heise.de/rss/heise.rdf (alt, manchmal tot)
//  Wir nehmen die Atom-Version als Primär, mit Fallback.
// ═══════════════════════════════════════════════════════════════

const RSS_FEEDS = [
  // 🇩🇪 Deutsch (10) — Heise-URL korrigiert
...
// Massive AI-Keyword-Liste (wie deine Version)
...
// ── NEU: Statistik für Sanity-Report ───────────────────────────
const feedStats = [];
...
const fetchPromises = RSS_FEEDS.map(async (feed) => {
  ...
  try {
    ...
    feedStats.push({ source: feed.source, rawCount: (xml.match(/<(?:item|entry)[\s>]/g) || []).length, kiCount: items.length, durationMs: duration, status: 'ok' });
    return items;
  } catch (error) {
    const duration = Date.now() - startTime;
    feedStats.push({ source: feed.source, rawCount: 0, kiCount: 0, durationMs: duration, status: 'error', error: error.message });
...
// ── SANITY-REPORT (wichtig für Debugging) ──────────────────────
const okFeeds = feedStats.filter(f => f.status === 'ok');
const deadFeeds = feedStats.filter(f => f.status === 'error');
const emptyFeeds = feedStats.filter(f => f.status === 'ok' && f.kiCount === 0);

console.log(`\n═══ RSS SANITY REPORT ═══`);
console.log(`Total feeds: ${RSS_FEEDS.length}`);
console.log(`✅ OK & lieferten KI-Items: ${okFeeds.filter(f => f.kiCount > 0).length}`);
console.log(`⚠️  OK aber 0 KI-Items: ${emptyFeeds.length} → ${emptyFeeds.map(f => f.source).join(', ')}`);
console.log(`❌ Fehler/Dead: ${deadFeeds.length} → ${deadFeeds.map(f => `${f.source} (${f.error})`).join(', ')}`);
console.log(`📊 Total KI-relevante Items: ${allItems.length}`);
